...
import { fetchH5LotteryStatus, getOrCreateParticipation } from "@/lib/h5/api";
...
export default function LotteryPage() {
  ...
  const { data: lottery, isLoading } = useQuery({
    queryKey: ["h5-lottery-status", campaignId, participationInfo?.participationId],
    queryFn: () => fetchH5LotteryStatus({
      campaignId,
  ...
  const chances = lottery?.myChances ?? 0;
        ...
        <div className="relative">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">
            <div className="rounded-2xl bg-white/10 p-3">
              ...
              <p className="text-2xl font-black">{lottery.dailyQuota}</p>
              <p className="text-[11px] text-slate-300">今日名额</p>
            </div>
            <div className="rounded-2xl bg-white/10 p-3">
              <Users className="mb-2 size-5 text-orange-200" />
              <p className="text-2xl font-black">{lottery.entryCount}</p>
        ...
        <CardContent className="space-y-4 p-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant={chances > 0 ? "success" : "warning"}>
              {chances > 0 ? statusText(lottery.status) : "待审核"}
            </Badge>
          </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">{lottery.drawTime ?? "以门店通知为准"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-orange-100 bg-orange-50 p-4 text-sm leading-6 text-slate-700">
            ...
            <p className="mt-1">第三关质量审核达到 60 分即可入池；60-79 分 1 次机会，80 分以上 2-3 次机会。</p>
          </div>
